...
- 支持跟随 iOS 系统切换浅色 / 深色模式，并保护主页面顶部安全区域
...
项目采用奶油黄 / 粉色可爱风：粉白背景、大圆角卡片、胶囊按钮、轻柔小票卡片、豆豆和 SF Symbols 图标。界面颜色使用 Asset Catalog 语义资源统一管理，支持跟随 iOS 系统自动切换浅色 / 深色模式；深色外观使用深莓色表面与更明亮的收支语义色。
...
点击“解析账单”后，App 会调用用户自己配置的 OpenAI 兼容接口，把自然语言解析为金额、类型、类目、备注、日期和置信度。如果一句话里包含多笔独立账单，App 会按发生顺序展示多张确认记录，例如工资入账和外卖出账会分成两笔。解析结果会先展示为确认卡片，用户必须点击“确认记账”或“确认记账 N 笔”才会写入本地 `LedgerRecord`。如果需要修改，可以点单笔记录里的“调整”，在确认前修改金额、类型、类目、备注和日期；调整过程不会提前写入账本。点“先不记”则不会保存。
...
源码继续维护在 `main` 分支。每次功能迭代完成后，会先更新源码和文档并推送到 `main`，再创建版本 tag，例如 `v1.0.0`、`v1.1.0`、`v1.2.0`、`v1.3.0`、`v1.3.1`、`v1.4.0`、`v1.5.0`。
