...
点击“解析账单”后，App 会调用用户自己配置的 OpenAI 兼容接口，把自然语言解析为金额、类型、类目、备注、日期和置信度。解析结果会先展示为确认卡片，用户必须点击“确认记账”才会写入本地 `LedgerRecord`。如果需要修改，可以点“手动调整”进入原来的“记一笔小账”页面；点“先不记”则不会保存。
...
- API Base URL 默认是 `https://cc.zaizai.pet:8888/v1`
- 备用 Base URL 默认是 `https://cc.zaizai.uno:8888/v1`
...
请检查 API Base URL、备用 Base URL、API Key、模型名称和网络连接。主地址失败后，如果开启了“主地址失败后自动使用备用地址”，App 会自动重试一次备用地址；如果备用地址也失败，会把失败原因显示出来。
